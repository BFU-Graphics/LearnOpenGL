...
///右上角立方体正面的顶点被镜面高光照亮。
/// 因为右下三角形的右上顶点是亮的和三角形的其他两个顶点是不亮的，亮的值插值到其他两个顶点。
/// 相同的情况发生在左上角三角形上。由于中间片段的颜色不是直接来自光源但都是插值的结果，在中间碎片和左上角的照明是不正确的
/// 右下角的三角形在亮度上发生碰撞，在两个三角形之间形成一条可见的条纹。
...
auto main() -> int
{
    ...
    Shader shader(MY_SHADER_DIR + std::string("01task3_shader_vertex.glsl"), MY_SHADER_DIR + std::string("01task3_shader_fragment.glsl"));
