...
auto main() -> int
{
    ...
    Shader shader(MY_SHADER_DIR + std::string("00shader_vertex.glsl"), MY_SHADER_DIR + std::string("00shader_fragment.glsl"));
    Shader lightShader(MY_SHADER_DIR + std::string("00light_shader_vertex.glsl"), MY_SHADER_DIR + std::string("00light_shader_fragment.glsl"));

    ///虽然还是写了 color和 texture coords属性，但是 shader中并没有用到，写在这也没关系主要是为了能和绑定函数中步长相契合
