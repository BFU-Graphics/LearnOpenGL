#ifndef LEARNOPENGL_OBJECT_H
#define LEARNOPENGL_OBJECT_H

#include <glad/glad.h>
#include <GLFW/glfw3.h>

#include <string>
#include <array>
#include <vector>

#include "shader.h"

class Object{
public:
    unsigned int ID;

public:
    Object(float vertices_array[], int vertices_array_size, unsigned int indices_array[], int indices_array_size);
    Object(float vertices_array[], int vertices_array_size);

public:
    void render(const Shader &shader); // 使用这个shader来渲染这个物体
    void renderCube(const Shader &shader);

public:
    struct Vertex
    {
        std::array<float, 3> position;
        std::array<float, 3> color;
        std::array<float, 3> tex_coords;
    };

private:
    std::vector<Vertex> vertices;
    std::vector<unsigned int> indices;
};

#endif //LEARNOPENGL_OBJECT_H
